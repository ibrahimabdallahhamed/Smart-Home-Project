...
u8 I2C_Read(void) {
    u8 returedData = 0;
    if (SSPCON1bits.SSPOV == 1 || SSPCON1bits.WCOL) // Bus Collision or Buffer Overflow
    {
        u8 Dummy = SSPBUF; // Read The Last Byte To Clear The Buffer
        SSPCON1bits.SSPOV = 0; // Clear the overflow flag
        SSPCON1bits.WCOL = 0; // Clear the collision bit
        I2C_CLOCK_STRETCH_DISABLE();
        // Release Clock Line SCL
    }
    while (!PIR1bits.SSPIF); // Wait until a byte is received
    ...
    returedData = SSPBUF; // Return received data
    I2C_CLOCK_STRETCH_DISABLE();




    return returedData; // If master is not writing, return 0 or handle differently
}